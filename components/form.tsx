...
export default function Form() {
    ...
    const getButtonClass = () => {
        var cls = "bg-main transition-colors text-lt h-[50px] rounded-md"
        if (showError) {
            cls += " bg-red-500"
        } else if (showSuccess) {
            cls += " bg-green-400"
        } else if (formIsValid()) {
            cls += " hover:bg-main-600"
        }
        return cls
    }
    ...
    return <div className="p-8 rounded-md bg-dk-500 w-fit space-y-8 lg:max-w-2xl text-lt">
        <h4 className="text-2xl md:text-4xl font-bold">Request More Information</h4>
        <p>Would you like to speak to us about our AI services? Just submit your details and we’ll be in touch shortly. You can also email us directly if you would prefer – <span className="font-bold">contact@portlandai.io</span></p>
        <div className="">
            <Input
                underlined
                fullWidth
        ...
        <div className="">
            <Input
                ...
                color="secondary"
                value={company}
                status={company == "" ? "default" : "primary"}
        ...
        <div className="">
            <Input
                ...
                color="secondary"
                value={email}
                status={emailValid() ? email == "" ? "default" : "primary" : "error"}
